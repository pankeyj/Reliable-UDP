...
void resend(frame *old, void *arg)
{
	...
	int j;
	char newsendBuffer[256];
	int clientSocket = *(int*)arg;
	struct swphdr *newhdr;
	newhdr  =(struct swphdr*)  calloc('0',sizeof(struct swphdr));

	newhdr->seqNum = old->seqNum;
	cout << "test" << endl;
	if(old->isLast)
		newhdr->flag = 3;
	else
		newhdr-> flag = 0;
	memcpy(newsendBuffer, newhdr, sizeof(struct swphdr));
	memcpy(newsendBuffer+HDR_LENGTH, old->data, sizeof(old->data));


	newhdr->chksum = ip_checksum((const void*)newhdr,12);
        cout << "CheckSum value" << newhdr->chksum << endl;
        memcpy(newsendBuffer, (void*)newhdr, sizeof(struct swphdr));

	sendto(clientSocket, newsendBuffer,256,
